...
test.describe("installed PWA account navigation", () => {
  ...
  for (const source of ["zilch", "zdwa"]) {
    test(`${source} PWA keeps the other game's account after switching`, async ({ page, baseURL }) => {
      ...
      const socialSettings = page.locator('details[data-account-section="social"]');
      await expect(socialSettings).not.toHaveAttribute("open", "");
      await socialSettings.locator(":scope > summary").click();
      ...
      await expect(page.getByRole("link", { name: "Spielerauswahl im Konto verwalten", exact: true })).toHaveAttribute("href", `/${destination}/konto?allowlist=1#settings`);
      if (source === "zilch") {
        ...
        await expect(page.getByRole("button", { name: "Zur Spielerauswahl hinzufügen", exact: true })).toBeEnabled();
        await expect(page.getByRole("link", { name: "Spielerauswahl im Konto verwalten", exact: true })).toHaveAttribute("href", "/konto?allowlist=1#settings");
      }
    });
  }
});
