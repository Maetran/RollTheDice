...
function playerAvatarMarkup(player, { avatarKey = "", showAdminBadge = false } = {}){
  return avatarMarkup(player, { avatarKey, showAdminBadge });
}

function playerNameMarkup(player, { name, compactRank = false, showRank = true, fallback = "Spieler", profileLink = false, avatarKey = "", showAdminBadge = false } = {}){
  ...
  return `<span class="player-name-with-rank">${playerAvatarMarkup(player, { avatarKey, showAdminBadge })}${nameMarkup}${showRank ? playerRankMarkup(player, { compact: compactRank, owner: label }) : ""}</span>`;
}
